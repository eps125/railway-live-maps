...
import { runImportCorpus } from "./corpusImporter.js";
...
describe("runImportCorpus (integration)", () => {
  beforeAll(async () => {
    await resetFixtureState("corpus-small.json");
    await pool.query("delete from location_reference where tiploc in ('LANCSTR', 'PRST')");
  });
  ...
  it("upserts location_reference rows by TIPLOC", async () => {
    const result = await runImportCorpus(deps, join(fixturesDir, "corpus-small.json"));

    expect(result.alreadyImported).toBe(false);
    expect(result.upsertedRows).toBe(2);

    const rows = await pool.query<{ tiploc: string; crs: string | null }>(
      "select tiploc, crs from location_reference where tiploc in ('LANCSTR', 'PRST') order by tiploc",
    );
    expect(rows.rows).toEqual([
      { tiploc: "LANCSTR", crs: "LAN" },
      { tiploc: "PRST", crs: "PRE" },
    ]);
  });

  it("reimporting byte-identical content is a safe no-op", async () => {
    const result = await runImportCorpus(deps, join(fixturesDir, "corpus-small.json"));
  ...
  it("a whole-file JSON parse failure marks the import failed rather than partially applying", async () => {
    await resetFixtureState("corpus-malformed.json");
    await expect(
      runImportCorpus(deps, join(fixturesDir, "corpus-malformed.json")),
    ).rejects.toThrow();

    const body = await readFile(join(fixturesDir, "corpus-malformed.json"));
    const checksum = sha256Hex(body);
