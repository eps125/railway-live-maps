...
5. Never assume a four-character berth description uniquely identifies a train run. **(Reinstated — ADR 0006, 2026-09-13, Milestone 34: candidate schedules are position-scoped to this berth's SMART-derived STANOX(es) before any tie-break runs; only a berth with no SMART coverage falls back to an unscoped nationwide headcode match, explicitly labelled the weakest `headcode_only` tier.)**
6. TRUST activation is the authoritative link between a run and a schedule when available. **(As of ADR 0002, this link comes from garner's `trust_activation.cif_schedule_id`, mirrored — not RLM's own activation-linking. ADR 0006/Milestone 34 made this the actual top-priority `matchBasis` tier, not just a same-day tie-break.)**
