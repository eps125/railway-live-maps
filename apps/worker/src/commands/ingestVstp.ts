...
/**
 * The live Network Rail VSTP connector. Refuses to start unless VSTP_LIVE_ENABLED=true and
 * NR_USERNAME/NR_PASSWORD (or _FILE variants) are set — same gating discipline as
 * `ingestTd.ts`: only enable after `worker replay-fixtures` and the integration suite have
 * passed. Not exercised against a live broker in this development environment.
 */
export async function runIngestVstp(config: Config): Promise<never> {
  if (!config.VSTP_LIVE_ENABLED) {
    throw new Error(
      "ingest-vstp requires VSTP_LIVE_ENABLED=true. Run `worker replay-fixtures` and the " +
        "integration suite first — see docs/IMPLEMENTATION_PLAN.md Milestone 7.",
    );
  }
  if (!config.NR_USERNAME || !config.NR_PASSWORD) {
    throw new Error(
      "ingest-vstp requires NR_USERNAME/NR_PASSWORD (or the _FILE variants) to be set",
    );
  }
  ...
  await connection.start({
    ...
    onSessionEnd: async (info) => {
      ...
      console.log(`VSTP session ended: ${info.disconnectReason}`);
    },
    onFrame: async (handle) => {
      const result = await recordVstpFrame(handle.frame, {
        pool,
        archiveClient,
    ...
    onError: (error) => {
      console.error("VSTP connection error:", error);
    },
  });
