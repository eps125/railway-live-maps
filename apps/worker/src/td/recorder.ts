 ...
 * this resolves, then calls markFrameAcked. Notifying projectors after commit is out of
 * scope here — that's a later milestone's extension point.
 */
export async function recordFrame(
  ...
  try {
    ...
    const parsed = parseTdFrame(frame.body, { receivedAt: frame.receivedAt });
